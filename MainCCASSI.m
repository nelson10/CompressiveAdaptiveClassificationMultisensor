...
M2 = size(RGB1,1); % spatial dimension multispectral
N2 = size(RGB1,2); % spatial dimension multispectral
M1 = size(RGB2,1); % spatial dimension hyperspectral
N1 = size(RGB2,2); % spatial dimension hyperspectral
...
for i=1:shot1
    t1 = T1(:,:,:,i);
    for j=1:L2
        YM(:,1+(j-1):M2+(j-1),i) = YM(:,1+(j-1):M2+(j-1),i) + (t1(:,:,j).*MS(:,:,j));
    end
    %if(i==1 && adaptive ==1)
       % YM(:,:,1)=RGB(:,:,2);
    %end
...
for i=1:shot2
    t2 = T2(:,:,:,i);
    for j=1:L1
        YH(:,1+(j-1):M1+(j-1),i) = YH(:,1+(j-1):M1+(j-1),i) + (t2(:,:,j).*HS(:,:,j));
    end
